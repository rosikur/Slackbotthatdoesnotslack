...
app.command("/slackbot-that-doesnt-slack-ping", async ({ command, ack, respond }) => {
...
app.command("/slackbot-that-doesnt-slack-help", async ({ ack, respond }) => {
  await ack();
  await respond({
    text:
`Available Commands:
...
app.command("/slackbot-that-doesnt-slack-joke", async ({ ack, respond }) => {
  await ack();
